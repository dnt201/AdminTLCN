...
import { ChevronUp, ChevronDown, More, FaceSmile, Photo, ArrowPath } from '@icons/index';
...
const CommentItem: React.FC<iProps> = (props) => {
    ...
    const [countReplyState, setCountReplyState] = useState(parseInt(countReply) || 0);
    const [pagingListReply, setPagingListReply] = useState<iPage>();
    const [curPage, setCurPage] = useState(1);
    const [valueComment, setValueComment] = useState(content);
    const [valueCommentFromDataBase, setValueCommentFromDataBase] = useState(content);
        ...
        if (curPage === -1) setCurPage(1);
            ...
            const fetchList = async () => {
                ...
                if (result.status === 200 || result.status === 201) {
                    console.log(result);
                    if (listReply !== null) setListReply([...listReply, ...result.data.data]);
                    else setListReply(result.data.data);
                    setPagingListReply(result.data.page);
                } else toast.error('Lấy danh sách reply comment lỗi');
            };
            fetchList();
        }
    }, [commentId, curPage]);
    ...
    useEffect(() => {
        ...
        return () => {
            document.removeEventListener('click', handleClickOutActionModalComment, true);
        };
    }, [refDivActionModal]);

    return (
        <div className='p-4 bg-[#f1f1f1] rounded-md mb-4 border-gray' id={commentId}>
            {/* Start header */}
            ...
            {isEditComment ? (
                ...
                <div className=' p-4   mt-4 py-2   border-gray border-[1px] rounded-md'>
                    <div className=' flex flex-col rounded-md'>
                        {/* Start Header */}
                        <div>
                            <button className=' px-2 border-b-2 mr-2 border-primary'>Viết</button>
                            <button
                                className=' px-2'
                                onClick={() => {
                ...
                <>
                    ...
                    <div className=' flex items-center'>
                        <button className='p-[2px]' data-tip='Vote Up' data-for='voteUpComment'>
                        ...
                        <button className='p-[2px]' data-tip='Vote Down' data-for='voteDownComment'>
            ...
            {listReply && listReply.length > 0 && (
                <div>
                    ...
                    {pagingListReply &&
                    curPage < Math.ceil(pagingListReply?.totalElement / pagingListReply.size) ? (
                        <button
