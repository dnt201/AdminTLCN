...
const postApi = {
    ...
    getListPostHaveBeenFollow: (name?: string, pageNumber?: number) => {
        let url = ``;
        if (name !== undefined && name !== '') url = `/post/all-post-follow?name=${name}`;
    ...
    getPostNeedApprove: (pageNumber?: number, name?: string, size?: number) => {
        console.log('not approve');
        const url = `/post/all-need-approve${name}`;
        return axiosClient.post(url, {
            size: size || 5,
            pageNumber: pageNumber || 1,
        });
    ...
    getAllPostByPostTag: (postTags: string[], pageNumber?: number, size?: number) => {
